...
import { getFavorites, getMovieDetail, getWatchList } from "../../api/api";
...
const MovieDetail = () => {
  ...
  const handleAddFavorites = async (tvId) => {
    const options = {
      ...
      data: { media_type: "movie", media_id: tvId, favorite: true },
    };
  ...
  useEffect(() => {
    const getFavoriteMovies = async () => {
      try {
        const res = await getFavorites("movies");
        if (res.data.results) {
          const favoriteList = res.data.results;
          setFavoriteMovies(favoriteList);

          const num = favoriteList.filter((item) => item.id == id);
          if (num.length > 0) {
            setAdd(false);
          }
        }
  ...
  const handleaAddWatchList = async (tvId)=>{
    const options = {
        ...
        data: {media_type: 'movie', media_id: tvId, watchlist: true}
      };
      try {
  ...
  useEffect(()=>{
    const getListWatchList = async()=>{
        try {
            const res = await getWatchList('movies');
            if(res.data.results){
          ...
          <div className={styles.details}>
            ...
            <div className={styles.info}>
              ...
              <ul className={styles.detailList}>
                ...
                <li className={styles.detailListItem}>
                  <span className={styles.detailListItemCaption}>Run time</span>
